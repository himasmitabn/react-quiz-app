  ...
  return (
    ...
    <div className="px-4  py-6 lg:px-8 lg:py-8">
        ...
        {options.map((option, index) => (
          <li key={index} className="flex items-center py-2 lg:py-3">
            <label className="flex items-center cursor-pointer">
              ...
              />
              <span className="ml-2 text-black-700 lg:ml-3">{option}</span>
            </label>
          </li>
        ))}
      </ul>
      <button
        ...
        className={`mt-4 py-2 px-4 rounded-lg text-sm font-medium lg:text-base ${selectedOption ? 'bg-blue-500 text-white' : 'bg-gray-500 text-black-700 cursor-not-allowed'}`}
      >
        Submit
      </button>
    </div>
